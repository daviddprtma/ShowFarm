  ...
  async updateUser(userId, updates) {
    try {
      ...
      if (updates.totalEntries !== undefined)
        allowedUpdates.total_entries = updates.totalEntries;
      if (updates.totalBadges !== undefined)
        allowedUpdates.total_badges = updates.totalBadges;
      if (updates.learningStreak !== undefined)
        allowedUpdates.learning_streak = updates.learningStreak;
...
if (supabase) {
  supabaseService.initializeTables().catch((err) => {
    ...
    console.log(
      "💡 Run the database_schema_complete.sql file in your Supabase SQL Editor",
    );
  });
} else {
  console.warn("⚠️ Supabase not configured - using local storage only");
